package com.apusic.ebiz.framework.core.model;
...
public abstract class IdEntity implements Serializable {
    ...
    */
	...
	/**
	 * 如果使用此方法，那么，属性的约束只能是以javax.validation.constraints包下的。
	 */
	public IdEntity validate() {
		return validate(validator);
	}
	/**
	 * 暂时使用Spring注入的validator，会有取不到{min}的现象,建议直接用validate()
	 * @param validator 
	 * @return
	 */
	@Deprecated
	public IdEntity validate(Validator validator) {
		...
		if (StringUtils.isNotBlank(__errorMessage)) {
			__errorMessage=__errorMessage.substring(0,__errorMessage.length()-1);
