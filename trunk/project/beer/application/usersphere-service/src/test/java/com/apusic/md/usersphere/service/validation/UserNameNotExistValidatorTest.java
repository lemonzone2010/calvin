...
@Transactional
public class UserNameNotExistValidatorTest{

	@Autowired
	@Qualifier("usersphere_UserNameNotExistValidator")
	...
	public void validateNotUserExists(){
		...
		Assert.assertEquals("UserSphere-Err-004", report.getCode());
		Assert.assertTrue(ValidationReport.Severity.ERROR == report.getServerity());
	}
	...
	public void validateUserExist(){
		...
		User user = new User();
		user.setName("hialex");
		user.setPassword("hialex");
		userService.addUser(user);
		ValidationReport report = validator.validate("hialex");
