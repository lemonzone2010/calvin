...
public class HibernateDocBinder {
	...
	private List<DocField> collectInfo(Class clazz) {
		...
		if (superClazz.isAnnotationPresent(Table.class)) {
			tableName=((Table)superClazz.getAnnotation(Table.class)).name();//generate tablename
		}
		...
		for (AccessibleObject member : members) {
			if (member.isAnnotationPresent(Column.class)) {
				...
				fields.add(new DocField(member,tableName));
			}
		}
		return fields;
	}
	...
	private static class DocField {
		...
		public DocField(AccessibleObject member,String tableName) {
		...
		private void storeName(Column annotation) {
			...
			// dynamic fields are annotated as @Field("categories_*")
				...
				// if the field was annotated as a dynamic field, convert the
				// name into a pattern
				// the wildcard (*) is supposed to be either a prefix or a
				// suffix, hence the use of replaceFirst
					...
					if (field.getGenericType() instanceof ParameterizedType) {
						...
						if (types != null && types.length == 2 && types[0] == String.class) {
							...
							if (types[1] instanceof Class) {
								// the value could be multivalued then it is a
								...
								if (types[1] == Collection.class || types[1] == List.class
									...
									// else assume it is a primitive and put in
									// the source type itself
									type = (Class) types[1];
								}
							}
							// Of all the Parameterized types, only List is
							// supported
		 ...
		 * This method supports reading of all "matching" fieldName's in the
		 * <code>SolrDocument</code>
		 * 
		 * Returns <code>SolrDocument.getFieldValue</code> for regular fields,
		 * and <code>Map<String, List<Object>></code> for a dynamic field. The
		 * key is all matching fieldName's.
		 */
		...
		public Object get(final Object obj) {
			if (field != null) {
				...
				throw new RuntimeException("Missing getter for field: " + name
