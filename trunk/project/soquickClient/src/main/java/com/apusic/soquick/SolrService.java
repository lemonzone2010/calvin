...
public class SolrService {
	...
	public <T> Page<T> query(String keyword, Class<T> clazz)
...
//		query.setTerms(true);
		...
		/*query.addFacetField("F_Table.F_publishedDate");
		Calendar calendar = Calendar.getInstance(Locale.UK);
	    calendar.set(2010, 1, 1);
	    Date start = calendar.getTime();
	    calendar.set(2011, 1, 1);
	    Date end = calendar.getTime();
	    query.addDateRangeFacet("F_Table.F_publishedDate", start, end, "+1MONTH");*/
	...
	public <T> Page<T> query(Query querys)
			throws SolrServerException, IOException {
		Page<T> ret = new Page<T>();
		...
		System.out.println(rsp.getResults());
		List<T> beans = getBinder().getBeans(querys.getQueryClazz(),rsp.getResults());
