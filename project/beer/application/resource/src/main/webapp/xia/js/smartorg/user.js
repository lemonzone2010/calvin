...
$(document).ready(function() {
	  ...
	  crudManager.selectedOneStatus=function(){
		...
		crudManager.customEditValidate=function(){
			if($('input[cid=confirmpassword]').val()!=$('#password').val()){
				alert('两次密码必须输入相同。');
				return false;
			}
			return true;
		...
		crudManager.editUserAfter=function(){
			$('#password').val('');
		}
	...
	*/
	...
	function grantRole(){
		...
		if(roles){
			$.post(contextPath+"/user/grant",{roleids:ids,userIds:crudManager.getSelectedIds(),type:$("#type").val()}, function(data) {
