...
public class SolrDispatchFilter implements Filter
{
  ...
  /**
	 * 将自己定义的solr home复制到 临时文件夹目录下，如果存在，则不复制
	 */
	private void initSolrHome() {
		StringBuffer userHome = new StringBuffer(System.getProperty("user.home"));
		home: {// TODO Modified by xiayong,DO not overwrite configure
				// eachtime,just get it for the first time.
			...
			if (userHome.append("\\.solr\\").toString().equalsIgnoreCase(solrHome)) {
				try {
					...
					String solrTemplatePath = this.getClass().getResource("/solrTemplate/").getPath();
					File solrFromXml = new File(solrTemplatePath + "solrTemplate.xml");
					File solrToXml = new File(solrHome + "/solr.xml");
					FileUtils.copyFile(solrFromXml, solrToXml);

					File fromDir = new File(solrTemplatePath + "/confTemplate");
					File toDir = new File(solrHome + "/conf/");
					FileUtils.copyDirectory(fromDir, toDir);
				} catch (Exception e1) {
					log.error("初始化solr home时出错",e1);
					throw new RuntimeException("初始化solr home时出错:"+e1.getMessage(), e1);
				}
			}
		}
	}
